...
//        }])->sum('value');
    ...
    {
        $data = ContractPayment::whereIn('contract_id', $this->userable->contracts->pluck('id'))->where('paid', 0)->whereNull('submit_date')->whereNotNull('date')->orderBy('date', 'desc')->limit(1)->first();
//        dd($data, $data->pluck('date'));
        $payment = $data ? $data->date : null;
        return $payment;
    }
    ...
    public function getIsRatableAttribute()
    {
        return Contract::where(function ($q) {
                $q->orWhere(function ($qu) {
                    $qu->where('owner_id', $this->userable->id)
                        ->where('tenant_id', auth('api')->user()->userable->id);
                })->orWhere(function ($qu) {
                    $qu->where('tenant_id', $this->userable->id)
                        ->where('owner_id', auth('api')->user()->userable->id);
                });
            })
                ->count() &&

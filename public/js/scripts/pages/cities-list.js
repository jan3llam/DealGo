...
$(function () {
    ...
    var dtTable = $('.cities-list-table'),
        newSidebar = $('.new-city-modal'),
        newForm = $('.add-new-city');
    ...
    if (dtTable.length) {
        dtTable.dataTable({
            ajax: assetPath + 'api/admin/cities/list',
            columns: [
            ...
            buttons: [
                ...
                {
                    text: 'إضافة جديد',
                    className: 'add-city btn btn-primary',
                    attr: {
    ...
    if (newForm.length) {
        ...
        newForm.on('submit', function (e) {
            ...
            if (isValid) {
                ...
                $.ajax({
                    type: 'POST',
                    url: assetPath + 'api/admin/cities/' + type,
                    dataType: 'json',
                    processData: false,
                    contentType: false,
                    data: data,
    ...
    $(document).on('click', '.item-delete', function () {
        ...
        $.ajax({
            type: 'DELETE',
            url: assetPath + 'api/admin/cities/' + element.data('id'),
            dataType: 'json',
            success: function (response) {
                if (parseInt(response.code) === 1) {
                    dtTable.DataTable().ajax.reload();

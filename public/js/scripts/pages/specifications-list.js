...
$(function () {
    ...
    var dtTable = $('.specifications-list-table'),
        newSidebar = $('.new-specification-modal'),
        newForm = $('.add-new-specification');
    ...
    if (dtTable.length) {
        dtTable.dataTable({
            ajax: assetPath + 'api/admin/specifications/list',
            columns: [
            ...
            buttons: [
                ...
                {
                    text: 'إضافة جديد',
                    className: 'add-specification btn btn-primary',
                    attr: {
    ...
    if (newForm.length) {
        ...
        newForm.on('submit', function (e) {
            ...
            if (isValid) {
                ...
                $.ajax({
                    type: 'POST',
                    url: assetPath + 'api/admin/specifications/' + type,
                    dataType: 'json',
                    processData: false,
                    contentType: false,
                    data: data,
    ...
    $(document).on('click', '.item-delete', function () {
        ...
        $.ajax({
            type: 'DELETE',
            url: assetPath + 'api/admin/specifications/' + element.data('id'),
            dataType: 'json',
            success: function (response) {
                if (parseInt(response.code) === 1) {
                    dtTable.DataTable().ajax.reload();

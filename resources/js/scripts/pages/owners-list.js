    ...
    if (dtTable.length) {
        dtTable.dataTable({
            ajax: function (data, callback, settings) {
                ...
                $.ajax({
                    url: assetPath + 'api/admin/owners/list',
                    data: {
                        length: data.length,
                        lang: $('html').attr('lang'),
                        start: data.start,
            ...
            columns: [
                ...
                {data: 'id'},
                {data: 'user.full_name'},
                {data: 'user.city', orderable: false},
            ...
            columnDefs: [
                ...
                },
                {
                    targets: 4,
                    render: function (data, type, full, meta) {
                        return data ? data.name + ' - ' + (data.state ? data.state.name : '') : '-';
                    }
                },
                {
                    // Actions
            ...
            buttons: [
                ...
                {
                    text: LANG.AddNew,
                    className: 'add-owner btn btn-primary',
                    attr: {
        ...
        if (ids.length) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'DELETE',
                        url: assetPath + 'api/admin/owners/bulk',
                        data: {ids: ids},
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
    ...
    $(document).on('click', '.status-switcher', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: 'PUT',
                    url: assetPath + 'api/admin/owners/status/' + element.data('id'),
                    dataType: 'json',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                        Authorization: 'Bearer ' + $('meta[name="api-token"]').attr('content')

...
$(function () {
    ...
    var dtTable = $('.regions-list-table'),
        newSidebar = $('.new-region-modal'),
        newForm = $('.add-new-region');
    ...
    if (dtTable.length) {
        dtTable.dataTable({
            ajax: assetPath + 'api/admin/regions/list',
            columns: [
            ...
            buttons: [
                ...
                {
                    text: 'إضافة جديد',
                    className: 'add-region btn btn-primary',
                    attr: {
    ...
    if (newForm.length) {
        ...
        newForm.on('submit', function (e) {
            ...
            if (isValid) {
                ...
                $.ajax({
                    type: 'POST',
                    url: assetPath + 'api/admin/regions/' + type,
                    dataType: 'json',
                    processData: false,
                    contentType: false,
                    data: data,
    ...
    $(document).on('click', '.item-delete', function () {
        ...
        $.ajax({
            type: 'DELETE',
            url: assetPath + 'api/admin/regions/' + element.data('id'),
            dataType: 'json',
            success: function (response) {
                if (parseInt(response.code) === 1) {
                    dtTable.DataTable().ajax.reload();
